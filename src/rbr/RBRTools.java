...
public class RBRTools {
	public double globalAr=0.0;
	public HashMap<String, String> restrictions = new HashMap<>();
public HashMap<String, Double[]> Ar = new HashMap<>(); //[0] NFI; [1] Lpath; [2] DFERpath; [3] Ar
	...
	public ArrayList<String> getPairAr(String src)
	{
		...
		{
			if(!key.split("/")[0].equals(src))
				continue;
			
			Ars.add(key.split("/")[1]+": "+Ar.get(key)[3]);
		}
	...
	//Wrong
	public double getArMedian()
	{
		double arMedian=0.0;
		ArrayList<Double> ars = new ArrayList<>();
		
		for(Double[] values : Ar.values())
			ars.add(values[3]);
		
		Collections.sort(ars);
		
		//Just for odd collections
		arMedian=ars.get(Math.round(ars.size()/2));
	...
	public double getArStd()
	{
		...
		double ArAccum=0.0;
		for(Double[] values : Ar.values())
			ArAccum+=(values[3]*values[3]);
		
		dblmean= ArAccum/Ar.size();
		
		dblsigma = dblmean-(this.globalAr*this.globalAr); //Variance
		sigma=Math.sqrt(dblsigma); //Standard Deviation
		
		return sigma;
	}		
	
	public void getArPerPath(int Nct)
	{
		
		for (String key : Ar.keySet())
		    Ar.get(key)[3]=Ar.get(key)[0]*Ar.get(key)[2]*Nct;		    

	}
	
	public double getAr(String inputTrafficPath, ArrayList<ArrayList<Router>> paths, int Nct)
	{
		this.getNFI(inputTrafficPath);
		
		for(String a : Ar.keySet())
			System.out.println("keyset "+a);
		this.getLpath(paths);
		this.setDFER(paths);
		this.getArPerPath(Nct);
		
		
		double totalArAccum=0.0;
		for(Double[] values : Ar.values())
			totalArAccum+=values[3];
		
		this.globalAr= totalArAccum/Ar.size();
		return this.globalAr;
	}
	
	private void setDFER(ArrayList<ArrayList<Router>> paths)
	{
		for(ArrayList<Router> path : paths) 
		{
			String src=path.get(0).getNome();
			String dst=path.get(path.size()-1).getNome();
			double DFER = 0.0;
			
			if(!Ar.containsKey(src+"/"+dst))
				continue;
			
			for(int i=0; i<path.size()-1;i++)
				DFER+= path.get(i).getLink(path.get(i+1)).getDFER();
		
			Ar.get(src+"/"+dst)[2]=DFER;
		}
	}
	
	private void getLpath (ArrayList<ArrayList<Router>> paths)
	{		
		for(ArrayList<Router> path : paths) 
		{
			String src=path.get(0).getNome();
			...
			Ar.get(src+"/"+dst)[1]=(double)path.size()-1;
		}			
	}
	
	private void getNFI(String inputDir)
	{	
		try 
		{
			...
			if(inputFiles!=null) 
			{
				...
				{
					String src = input.getName().substring(4,6);
					String dst = "";
					long nFlit = 0; 
					Scanner inputSc = new Scanner(new FileReader(input));
					while(inputSc.hasNextLine()) 
					{
						inputSc.next();
						dst=inputSc.next().substring(2, 4);
						nFlit = Integer.parseInt(inputSc.next(),16)+6;
						Double[] data ={(double)nFlit,0.0,0.0,0.0};
						if(!Ar.containsKey(src+"/"+dst))
						{
							Ar.put(src+"/"+dst,data);							
						}
	...
	private static void setLinkWeight(ArrayList<ArrayList<Router>> paths,
		...
		for (ArrayList<Router> path : paths) {
			for (int v = 0; v < path.size() - 1; v++) {
				 ...
				 * link.incremWeight();
				 */
				path.get(v).getLink(path.get(v + 1)).incremWeight();
